...
class TestBroadcast(TestCase):
    def test_listen_of_common(self):
        ...
        broadcast_service.publish("test_listen_of_common_specify_params", 11, 22, 33)
        wait()
        self.assertTrue(self.test_listen_of_common_specify_params)

        # test listen multi topics of common
        topics = [
            "test_listen_multi_topic1_of_common",
    ...
    def test_listen_of_decorator(self):
        ...
        def handle_topic_no_params_2():
            self.test_listen_of_decorator_no_params2 = True

        @broadcast_service.on_listen(
            [
                "test_listen_of_decorator_no_params1",
                "test_listen_of_decorator_no_params2",
            ]
        )
        def handle_topic_no_params_merge():
            if (
                self.test_listen_of_decorator_no_params1
                and self.test_listen_of_decorator_no_params2
            ):
        ...
        broadcast_service.publish("test_listen_of_decorator_specify_params", 11, 22, 33)
        wait()
    ...
    def test_listen_of_lambda(self):
        ...
        broadcast_service.subscribe(
            "test_listen_of_lambda_specify_params",
            lambda: handle_topic_specify_params(True),
        )
    ...
    def test_broadcast(self):
        ...
        broadcast_service.publish(
            ["test_broadcast_multi_topic1", "test_broadcast_multi_topic2"]
        )
        wait()
        self.assertTrue(
            self.test_broadcast_multi_topic1 and self.test_broadcast_multi_topic2
        )
    ...
    def test_broadcast_multiple_call_one_topic(self):
        ...
        broadcast_service.config(num_of_executions=5).publish(
            "test_broadcast_multiple_call_one_topic"
        )
        wait()
